...
void fdb_vertex_load(Vertex* vert, uint32_t id, uint8_t* source) {
    ...
    vert->firstEdgeId = letohu32(*((uint32_t*) (source + FDB_VERTEX_FIRSTEDGEID_OFFSET)));
...
void fdb_vertex_unload(Vertex* vert, uint8_t* dest) {
    ...
    uint32_t firstEdgeId = htoleu32(vert->firstEdgeId);

    memcpy(dest + FDB_VERTEX_SYMBOLID_OFFSET, &symbolId, 2);
    memcpy(dest + FDB_VERTEX_FIRSTEDGEID_OFFSET, &firstEdgeId, 4);
